...
app.post("/api/auth/register", async (req, res) => {
  try {
    ...
    return res
      .status(201)
      .json({ message: "User register successfully!", id: newUser._id });
  } catch (error) {
...
app.post("/api/auth/login", async (req, res) => {
  try {
    ...
    if(user['2faEnable']){
      ...
      cache.set(config.cacheTemporaryTokenPrefix + tempToken, user._id , config.cacheTemporaryExpireInSeconds)

      return res.status(200).json({ tempToken, expiresInSeconds: config.cacheTemporaryExpireInSeconds})
    }else{
...
app.post('/api/auth/login/2fa', async( req,res)=>{
  try{
    ...
    if(!tempToken || !totp){
      return res.status(422).json({ message: 'Please fill in all fields (tempToekn and totp)'})
    }
...
app.get(
  ...
  (req, res) => {
    return res
      .status(200)
      .json({ message: "Only admin and moderators can access this route!" });
  }
);
